...
(() => {
  ...
  const setup=(databaseConnected=null)=>{
    ...
    root.innerHTML=`<div class="auth-card"><div id="tnps-database-status" class="auth-status auth-status-checking"><span class="auth-status-dot"></span><span>Checking database…</span></div><h1>Sign in</h1><p>Use your TNPS email or ID. Role and permissions are loaded from the secure profile.</p><form id="tnps-login-form"><div class="auth-field"><label for="tnps-email">Email or ID</label><input id="tnps-email" type="text" inputmode="email" autocomplete="username" autocapitalize="none" required placeholder="Email or Teacher/Student ID"></div><div class="auth-field"><label for="tnps-password">Password</label><input id="tnps-password" type="password" enterkeyhint="go" autocomplete="current-password" required placeholder="••••••••"></div><button class="auth-btn" type="submit">Sign in</button><div id="tnps-auth-msg"></div></form></div>`;
    ...
    root.querySelector('form').onsubmit=async event=>{event.preventDefault();const identifier=email.value.trim();if(!identifier||!password.value){message(root,'Enter your email or ID and password.');(identifier?password:email).focus();return;}if(!supabaseConfigured){message(root,'TNPS authentication is not configured.');return;}setBusy(root,true);message(root,'Signing in…',true);try{const candidates=await resolveLoginEmails(identifier);if(!candidates.length){setBusy(root,false);message(root,'No active TNPS account was found for this email or ID.');email.focus();return;}let lastError=null;for(const loginEmail of candidates){const result=await supabase.auth.signInWithPassword({email:loginEmail,password:password.value});if(!result.error){await unlock(root);return;}lastError=result.error;}setBusy(root,false);message(root,lastError?.message==='Invalid login credentials'?'Invalid email/ID or password.':lastError?.message||'Unable to sign in.');password.focus();}catch(error){setBusy(root,false);message(root,error?.message||'Unable to sign in.');}};

...
(() => {
  ...
  const boot=async()=>{if(!supabaseConfigured){setup(false);window.dispatchEvent(new CustomEvent('tnps-database-ready',{detail:{connected:false,authenticated:false}}));return;}setup(null);checkSupabaseDatabaseService().then(connected=>{const root=document.getElementById(ID);if(root)setDatabaseStatus(root,connected);window.dispatchEvent(new CustomEvent('tnps-database-ready',{detail:{connected,authenticated:false}}));}).catch(()=>{const root=document.getElementById(ID);if(root)setDatabaseStatus(root,false);window.dispatchEvent(new CustomEvent('tnps-database-ready',{detail:{connected:false,authenticated:false}}));});try{const {data,error}=await Promise.race([supabase.auth.getUser(),new Promise((_,reject)=>window.setTimeout(()=>reject(new Error('Authentication bootstrap timeout')),5000))]);if(error||!data.user)return;const result=await getCurrentProfile();if(result.profile?.active&&['developer','principal','teacher','parent'].includes(result.profile.role)){localStorage.setItem('tnps-auth-role',result.profile.role);cache(result.profile,result.user);window.dispatchEvent(new CustomEvent('tnps-auth-ready',{detail:{role:result.profile.role}}));remove();}}catch{/* Login UI remains available. */}};

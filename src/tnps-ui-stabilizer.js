...
(() => {
  ...
  let connected=false,frame=0;
  const style=document.createElement('style');style.textContent=`body{overflow-x:hidden}.tn-db-status{display:inline-flex;align-items:center;gap:6px;margin-right:8px;color:var(--muted,#7d8799);font-size:10px;font-weight:700;white-space:nowrap}.tn-db-status i{width:7px;height:7px;border-radius:50%;display:block;background:#8b5cf6;box-shadow:0 0 0 3px rgba(139,92,246,.12)}.tn-db-status.off i{background:#c74e58;box-shadow:0 0 0 3px rgba(199,78,88,.12)}.tn-profile-avatar{display:grid;place-items:center;overflow:hidden}@media(max-width:680px){.tn-db-status{font-size:9px;margin-right:2px}.tn-db-status i{width:6px;height:6px}}`;document.head.appendChild(style);
  ...
  const apply=()=>{cleanupDashboard();renderStatus();fixSidebar()};

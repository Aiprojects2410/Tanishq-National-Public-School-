-- TNPS stabilization: security, performance, profile photos and attendance rules.
...
create index if not exists exam_marks_created_by_idx on public.exam_marks(created_by);
create index if not exists exam_marks_student_id_idx on public.exam_marks(student_id);
create index if not exists exam_marks_subject_id_idx on public.exam_marks(subject_id);
...
create index if not exists notices_published_by_idx on public.notices(published_by);
...
create index if not exists tickets_closed_by_idx on public.tickets(closed_by);
...
create index if not exists timetable_created_by_idx on public.timetable(created_by);
...
revoke execute on function public.calculate_exam_summary(uuid,uuid) from public;
grant execute on function public.calculate_exam_summary(uuid,uuid) to authenticated;
...
revoke execute on function public.set_ticket_status(uuid,text) from public;
...
grant execute on function public.get_public_qr_profile(text) to anon,authenticated;

create or replace function public.generate_ticket_number() returns trigger language plpgsql set search_path=public as $$ begin if new.ticket_number is null then new.ticket_number := 'TNPS-' || to_char(now(),'YYYY') || '-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,8)); end if; return new; end; $$;
